...
class Conv:
    ...
    def __init__(self,FiltersNum,FilterSize,Stride):
        ...
        self.filter=self.filter.astype('float32')
        #self.filter =np.zeros([FiltersNum,FilterSize,FilterSize],dtype='float32')
        self.bias = np.zeros(FiltersNum, dtype='float32')
        self.activ = act.Relu()
    ...
    def forward(self,input):
        ...
        out = np.zeros((b,self.FiltersNum,new_h,new_w), dtype='float32')
        for batch in range(b):
            for f in range(self.FiltersNum):
                for h in range(new_h):
                    for w in range(new_w):
                        patch = input[batch,:,h:(h+filter_h), w:(w+filter_w)]
                        out[batch,f,h,w] = np.sum(patch*self.filter[f], dtype='float32') + self.bias[f]
    
        self.ConvOut= self.activ.forward(out)
        return self.ConvOut
    
    def Backprop(self, pre_grad):
        self.dW = np.zeros(self.filter.shape, dtype='float32')
        self.db = np.zeros(self.bias.shape, dtype='float32')
        #relu derivative 
            ...
            for ds in range(d):
                for h in range(filter_h):
                    for w in range(filter_w):
                        ...
                        self.dW[f,h,w] = np.sum(input_window * delta_window, dtype='float32') / batch    

        for r in np.arange(self.FiltersNum):
            self.db[r] = np.sum(delta[:, r], dtype='float32') / batch
